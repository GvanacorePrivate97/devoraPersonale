...
@MainActor
@Observable
final class StaffProfileViewModel {
    ...
    func load() async {
        await state.run {
            ...
            user = me
            op = try await catalog.operators().first { $0.id == me?.operatorId }
        }
    }
...
/// "Profilo" dell'operatore: chi è loggato, i suoi contatti, la foto e
/// l'uscita dall'account.
struct StaffProfileScreen: View {
    @State var viewModel: StaffProfileViewModel
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 10) {
                    HStack(alignment: .bottom) {
                        BrandSectionLabel(text: L("staff_profile_account"))
                        Spacer()
                        if viewModel.user?.avatarPath != nil {
                            Button {
                                viewModel.onPhotoRemoved()
                            } label: {
                                Text(L("staff_profile_photo_remove"))
                    ...
                    VStack(spacing: 0) {
                        StoneKeyValueRow(label: L("staff_profile_email"), value: viewModel.user?.email ?? "")
